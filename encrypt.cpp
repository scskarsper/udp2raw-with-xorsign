...
int cipher_encrypt(const char *data,char *output,int &len,char * key)
{
        char timedom[500]="";
        char key2[16]="";
        sprintf(timedom,"%d", get_timekey());
        int iw,ik;
        ik=strlen(timedom);
        for (iw = 0; iw < 16; iw++) {
                int m1=key[iw];
                if(iw<ik)m1=m1+timedom[iw];
                m1=m1%255;
                key2[iw]=m1;
        }
	switch(cipher_mode)
	...
	case cipher_xor:return cipher_xor_encrypt(data,output,len, key2);
	case cipher_none:return cipher_none_encrypt(data,output,len, key);
	default:return cipher_xor_encrypt(data,output,len, key);
	}
...
int cipher_decrypt(const char *data,char *output,int &len,char * key)
{
	...
	switch(cipher_mode)
	{
		...
		case cipher_xor:return cipher_xor_decrypt(data,output,len, key2);
		case cipher_none:return cipher_none_decrypt(data,output,len, key);
		default:	return cipher_xor_decrypt(data,output,len,key);
	}
