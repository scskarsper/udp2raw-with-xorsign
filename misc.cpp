...
char key_string[1000]= "oh my gosh secret key";// -k option
...
void print_help()
{
	...
	printf("    --over-wait		  <number>	  set the interval of pwd change,default:3600\n");
	...
	static struct option long_options[] =
	  {
		...
		{"over-wait", required_argument,    0, 1},
   ...
   for(i=+1;i<argc;i++)
   {
	...
	while ((opt = getopt_long(argc, argv,options,long_options,&option_index)) != -1) {
		...
		case 1:
			...
			else if(strcmp(long_options[option_index].name,"over-wait")==0)
			{
				process_over_wait();
			}
